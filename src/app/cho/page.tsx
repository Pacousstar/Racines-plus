...
export default function ChoBoard() {
    ...
    useRoleRedirect(['cho', 'choa']);
    const [activeTab, setActiveTab] = useState<'tasks' | 'confirmed' | 'rejected' | 'ancestor' | 'team'>('tasks');
    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (!profile || !['cho', 'choa'].includes(profile.role)) { router.push('/dashboard'); return; }
    ...
    const isCHO = myProfile?.role === 'cho';
    ...
    const tabs = [
        ...
        ...(isCHO ? [{ key: 'ancestor', label: 'Ancêtre Village', icon: TreePine, count: 0, countColor: 'bg-purple-500' }] : []),
        ...(isCHO ? [{ key: 'team', label: 'Mon Équipe', icon: Users, count: 0, countColor: 'bg-blue-500' }] : []),
    ];
        ...
        <div className="min-h-screen bg-gray-50 text-foreground">
            ...
            <header className="fixed top-0 w-full bg-white border-b border-gray-100 px-6 py-3 flex justify-between items-center z-50 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border ${isCHO ? 'bg-purple-50 border-purple-200 text-purple-600' : 'bg-blue-50 border-blue-200 text-blue-600'}`}>
                        <ShieldCheck className="w-3.5 h-3.5" />
                        {isCHO ? 'CHO — Directeur du Patrimoine' : 'CHOa — Adjoint Quartier'}
                    </div>
                </div>
