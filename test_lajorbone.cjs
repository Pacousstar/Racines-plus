...
async function run() {
    ...
    try {
        ...
        const validateurId = (choas && choas.length > 0) ? choas[0].id : admin[0].id;
        const validateurId2 = (choas && choas.length > 1) ? choas[1].id : validateurId;

        console.log(`   🔍 Validateurs identifiés: [${validateurId}, ${validateurId2}]`);
        ...
        const { data: finalProfile } = await supabase.from('profiles').select('status, first_name, last_name, village_origin').eq('id', userId).single();
        console.log(`[1] Profil Final      : ${finalProfile.first_name} ${finalProfile.last_name} (${finalProfile.village_origin})`);
        console.log(`[2] CHO -> Patrimoine : ${finalProfile.status === 'confirmed' ? '✅ OK (Statut: confirmed)' : '❌ ERREUR'}`);
        ...
        console.log(`[3] Commentaires      : ${dbComms.length > 0 ? `✅ OK (${dbComms.length} commentaire trouvé)` : '❌ ERREUR'}`);
        ...
        console.log(`[4] Notifs is_read    : ${notifs.length > 0 && notifs[0].is_read === false ? '✅ OK (Notification non lue en base)' : '❌ ERREUR'}`);
        ...
        console.log(`[5] Table validations : ${validations.length > 0 ? `✅ OK (${validations.length} entrées)` : '❌ ERREUR'}`);
